...
// Drop in replacement for sin(x)
float FastSin(float d) {
 static const float kToDegs = 360.0 / (2.0 * PI);
 return isin(long(d * kToDegs));
}

float FastCos(float d) {
 static const float kToDegs = 360.0 / (2.0 * PI);
 return isin(long(d * kToDegs) + 90);
}
