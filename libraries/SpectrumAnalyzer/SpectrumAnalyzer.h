...
class SpectrumAnalyzer {
 public:
  ...
  struct Output {
    ...
    uint8_t spectrum_array[7];
    static const int spectrum_array_size = 7;
    Output() : spectrum_array() {
        memset(spectrum_array, 0, sizeof(spectrum_array));
    }
    
    void DebugPrint(HardwareSerial& s) {
      int anyPeaks = 0;
      for (int i = 0; i < spectrum_array_size; i++) {
        uint8_t val = spectrum_array[i];
        anyPeaks = anyPeaks || val > 1015;
  ...
  // Loudest of both channels is set to the equalizer. This gives good
  // behavior over mean equalizer for mono channels.
  void ProcessAudioMaxChannel(Output* mono_mixed);
